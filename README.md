# 🚀 QuantInsight-AI

AI-powered quantitative risk analysis platform combining technical indicators, real-time market data, and news-driven sentiment to generate explainable investment insights.

---

## 🔥 Features

- 📊 Real-time stock data (RSI, Moving Average)
- 🧠 AI-generated risk score & reasoning
- 📈 Multi-stock comparison dashboard
- 📰 News-based sentiment analysis
- 🎨 Interactive Streamlit UI

---

## 📷 Screenshots
<img width="1853" height="878" alt="image" src="https://github.com/user-attachments/assets/8733dc1d-af60-4871-a810-6661ab04a7da" />
<img width="1889" height="880" alt="image" src="https://github.com/user-attachments/assets/71ca927a-fe49-4d0a-819f-b8ee7524ef9b" />
<img width="1796" height="745" alt="image" src="https://github.com/user-attachments/assets/986d8bf6-c547-492d-88df-83f9f3113eb2" />
<img width="1858" height="619" alt="image" src="https://github.com/user-attachments/assets/25fb9ce4-9d10-4e22-9dfc-b88835ff35d6" />
<img width="1794" height="803" alt="image" src="https://github.com/user-attachments/assets/e07eadfa-e520-485e-84de-a1dc01a4f86f" />
<img width="1861" height="842" alt="image" src="https://github.com/user-attachments/assets/10ab7065-249a-4b7a-a427-2dc6ef35ac9d" />


## 🛠 Tech Stack

- Python
- Streamlit
- yfinance
- Plotly
- Gemini API
- NewsAPI

---

## ⚙️ How It Works

- Fetches stock data using yFinance  
- Processes and analyzes data using Pandas  
- Generates visualizations using Plotly  
- Displays results through an interactive Streamlit interface  

---

## 🌐 Live Demo

👉 https://quantinsight-ai.streamlit.app/

---

## ⚙️ Run Locally

```bash
git clone https://github.com/gargshivang257/QuantInsight-AI
cd QuantInsight-AI
pip install -r requirements.txt
streamlit run app.py
